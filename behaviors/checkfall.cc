...
bool NaoBehavior::checkingFall() {
    ...
    if(fallState == 1) {
        ...
        if (fallTimeWait < 0) {
            //cout<<"fallTimeWait < 0 "<<endl;
            fallTimeWait = worldModel->getTime();
        }

        if(worldModel->getTime() - currentFallStateStartTime > 0.2) { //TUNE origianl value is 0.2 
            //cout<< "worldModel->getTime() - currentFallStateStartTime > 0.2 "<<endl;
            fallState = 2;
            currentFallStateStartTime = -1.0;
        ...
        if(currentFallStateStartTime < 0) {
            currentFallStateStartTime = worldModel->getTime();
            //cout<<"fallstate=2................currentfFallStateStartTime < 0"<<endl;
        }
    ...
    if(fallenUp) {
        ...
        if(fallState == 3) {
            ...
            if (bodyModel->hasToe()) {  // we don't have toe for nao robot, ignore this 
            ...
            if (bodyModel->hasToe()) { //ignore this 
            ...
            // add by me using the magma parms 
            //bodyModel->setTargetAngle(EFF_LL2, atof(namedParams.find("getup_parms_stateUp9L2")->second.c_str()));
            //bodyModel->setTargetAngle(EFF_RL2, -atof(namedParams.find("getup_parms_stateUp9L2")->second.c_str()));
            /////end here 
            ...
            // add by me using the magma parms 
            //bodyModel->setTargetAngle(EFF_LL2, atof(namedParams.find("getup_parms_stateUp13L2")->second.c_str()));
            //bodyModel->setTargetAngle(EFF_RL2, -atof(namedParams.find("getup_parms_stateUp13L2")->second.c_str()));
            /////end here 
