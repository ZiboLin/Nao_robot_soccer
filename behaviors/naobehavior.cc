...
// find the number of opponents around the players, in a distance(radius) of d 
int NaoBehavior::opponentcount(VecPosition a, double distance)
{
    int opponent_counter = 0;
    for(int jj = WO_OPPONENT1; jj < WO_OPPONENT1+NUM_AGENTS; ++jj) 
    {
        //temp used to store the opponent individuals 
        VecPosition temp;
        ...
        //calculate the distance to our players 
        double distanceToplayer = temp.getDistanceTo(a); 
        //if the opponent are nearby , then increase the opponent-count
