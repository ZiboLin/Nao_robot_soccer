...
inline double Rad2Deg( double x ) //convertingradian to degree
{
    return ( x * 180 / M_PI );
}

inline double Deg2Rad( double x ) //converting degree to radian 
{
    return ( x * M_PI / 180 );
}
...
private:
    ...
    double m_z;   /*!< z-coordinate of this position */  // "z in here should represent the vertical position(in th air if z!=0 ) 
...
public:
    ...
    /*
       ...
       bool               operator ==            ( const double      &d           );
       */
    inline VecPosition operator - ( ) const { //operater overloaded
